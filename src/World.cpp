...
void World::stepSimulation() {
  int steps = engine.getDynamicsWorld()
      ->stepSimulation(1 / World::STEPS_PER_SECOND, World::MAX_STEPS);
...
void World::setAmbientColor(const glm::vec4 &color) {
  ambientColor = color;
}
  ...
  for (; index < dominoNumber; ++index) {
    ...
    world->addObject(
        boxBuilder.setTransform(btTransform(yRotation, currentPosition))
            ...
            .setShininess(51.2f).setSides(btVector3(0.25, 2.0, 0.5)).create());
  }
}
...
//                .create());
  ...
  //  // Sphere.
  //  world->addObject(sphereBuilder.setTransform(btTransform(btQuaternion::getIdentity(),
  //                                                          btVector3(-50.0,
  // 10.0, 0.0)))
  //                                .setMass(btScalar(20.0))
  //                                .setAmbientColor(btVector4(0.1745, 0.01175,
  // 0.01175, 1.0))
  //                                .setDiffuseColor(btVector4(0.61424, 0.04136,
  // 0.04136, 1.0))
  //                                .setSpecularColor(btVector4(0.727811,
  // 0.626959, 0.626959, 0.6))
  //                                .setShininess(76.8f)
  //                                .setRadius(btScalar(20.0))
  //                                .create());
  //  // Sphere.
  //  world->addObject(sphereBuilder.setTransform(btTransform(btQuaternion::getIdentity(),
  //                                                          btVector3(50.0,
...
//      btTransform(yRotation, btVector3(0.0, 0.0, 0.0))).setMass(btScalar(0.0))
//                .setAmbientColor(btVector4(0.1745, 0.01175, 0.01175, 1.0))
//                .setDiffuseColor(btVector4(0.61424, 0.04136, 0.04136, 1.0))
//                .setSpecularColor(btVector4(0.727811, 0.626959, 0.626959, 0.6))
...
void World::object_iterator::toNext() {
  ++currentObject;
  if (currentObject == objects.size()) currentObject = -1;
}
...
World::const_object_iterator::const_object_iterator(const const_object_iterator& original) {
  this->objects = original.objects;
  this->currentObject = original.currentObject;
}
...
bool World::const_object_iterator::operator!=(const const_object_iterator& iter) const {
  return iter.currentObject != this->currentObject;
}
...
void World::light_iterator::toNext() {
  ++currentLight;
  if (currentLight == lights.size()) currentLight = -1;
}
...
World::const_light_iterator::const_light_iterator(const const_light_iterator& original) {
  lights = original.lights;
  currentLight = original.currentLight;
}
...
bool World::const_light_iterator::operator!=(const const_light_iterator& iter) const {
  return iter.currentLight != this->currentLight;
}
